...
// Da spostare dentro un controller a parte
// In base al tipo di richiesta devo andare a vedere se è una GET/POST/PUT/PATCH
export const deleteCachedData: Handler = async (req, res) => {
    const query = req.query;
    const path = req.path;
    const fsManager = new fileSystemManager('Get');

    const splittedPath = fsManager.splitPath(path, 3);
    ...
    switch (type) {
        case ClearType.FOLDER:
            if (existsSync(baseUrl) && existsSync(`${baseUrl}/${folderPath}`)) {
                fsManager.removeRecursive(`${baseUrl}/${folderPath}`); //recursiveRemove(`${baseUrl}/${folderPath}`);
        ...
        case ClearType.CONTENT:
            if (existsSync(baseUrl) && existsSync(baseUrl + '/' + folderPath + '/' + fileName + '.json')) {
                ...
                fsManager.removeRecursive(`${baseUrl}/${folderPath}/${fileName}.json`);
